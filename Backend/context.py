...
class MemoryClient:
    ...
    def update_memory(query: str, response: str):
        ...
        last_memory_update: int = FileSystem.retrieve_json("systems.json")[
            "last_memory_update"]
        memory_threshold: int = FileSystem.retrieve_json("systems.json")[
            "memory_threshold"]

        # append memory to memory.txt
        if response != "":
            FileSystem.append_txt("storage/memory.txt",
                                  f"Query: {query}\nResponse: {response}\n")

            # update last memory update
            FileSystem.change_json_value(
                "systems.json", "last_memory_update", time.time())

            last_memory_update += 1
            FileSystem.change_json_value(
                "systems.json", "last_memory_update", last_memory_update % memory_threshold)
        else:
...
class DateTimeClient:
    ...
    def get_realtime_information_string():
        try:
            ...
            # Full name of the weekday (e.g., Tuesday)
            day = current_date_time.strftime("%A")
            # Day of the month as a zero-padded decimal number (e.g., 08)
            date = current_date_time.strftime("%d")
            # Full name of the month (e.g., April)
            month = current_date_time.strftime("%B")
            # Year with century (e.g., 2025)
            year = current_date_time.strftime("%Y")
            # Hour (24-hour clock) as a zero-padded decimal number (e.g., 20)
            hour = current_date_time.strftime("%H")
            # Minute as a zero-padded decimal number (e.g., 15)
            minute = current_date_time.strftime("%M")
            # Second as a zero-padded decimal number (e.g., 41)
